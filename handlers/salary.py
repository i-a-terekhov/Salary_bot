...
async def continue_to_create_password_report(callback: CallbackQuery):
    ...
    if command == 'Генерация':
        ...
        await get_secret_codes_for_employee(callback=callback, employee_codes=tuple_of_part_employee_codes)
        ...
        await callback.message.answer(text=f'Готовим секретные коды для сотрудников {part_of_employee}')
        await get_secret_codes_for_employee(callback=callback, employee_codes=list_of_all_employee_codes)
